...
public class FlowCrmErrorResponseHandler implements AccessDeniedHandler {
        ...
        if (!response.isCommitted()) {
            try {
                ...
                if (exception instanceof AbstractFlowCrmException) {
                    handleBookABusException((AbstractFlowCrmException) exception, response);
    ...
    private void handleBookABusException(AbstractFlowCrmException flowCrmException, HttpServletResponse response) throws IOException {
